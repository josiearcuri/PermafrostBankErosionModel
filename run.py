...
from src import boundaryconditionstools as bct
from src import watertemperaturemodel as wtm
...
def get_params():
    ...
    options['dt'] = 60*60*3
    options['dz'] = 0.01
    options['runtime'] =60*60*24*365*5
...
def run_model_scenario(params):
    ...
    bcdict= bct.BoundaryConditionsTools(params).get_bcs()
    bcdict= wtm.River_temp_model(bcdict)
    
    # run bank erosion model
    model = pbem.PermafrostBankErosionModel(params, bcdict)
    model.run()
    erosionrate = model.get_erosion()
    ...
    # plot results
    fig = model.plot_bankprofile(title = str("E = " + str(np.round(erosionrate,2))+ " m/yr"))
